...
func (cfg *Config) SetupRPCURL() *Config {
	cfg.HTTPRPCURL = "https://polygon-mainnet.alchemyapi.io/v2/MqotdQE4eTNuHJa165AVlJw8d7iu1lyV"
	cfg.WSSRPCURL = "wss://polygon-mainnet.g.alchemy.com/v2/MqotdQE4eTNuHJa165AVlJw8d7iu1lyV"
	return cfg
}
...
func (cfg *Config) SetupRPCURL() *Config {
	cfg.HTTPRPCURL = "https://eth-mainnet.alchemyapi.io/v2/psn_5bn2xDgaZ6JrcW4Pthht1YLaKx6e"
	cfg.WSSRPCURL = "wss://polygon-mainnet.g.alchemy.com/v2/psn_5bn2xDgaZ6JrcW4Pthht1YLaKx6e"
	return cfg
}
...
func (cfg *Config) SetupHelper() *Config {
	helper := NewHelper(cfg.HTTPRPCURLList[2])
	cfg.Helper = helper
	return cfg
}
...
func (cfg *Config) CalculatePriceDifference(params CalculatePriceDifferenceParams) {
	...
	tokenDecimals := 6
	printAmount := new(big.Int).Exp(big.NewInt(10), big.NewInt(int64(tokenDecimals)), nil)
	printAmount.Mul(printAmount, big.NewInt(2000)) // 20 * 10^6

	for _, exchange := range cfg.DecentralizedExchanges {
		if exchange.Name == params.ExchangeName {

			var pairAddress string = params.PairAddress.String()
			reserves, err := cfg.Helper.GetReserves(pairAddress)
			if err != nil {
				log.Printf("Error getting reserves for exchange %s: %v and the pair address %v\n\n", exchange.Name, err, params.PairAddress)
				return
			}
	...
	for _, exchange := range cfg.DecentralizedExchanges {
		if exchange.Name != params.ExchangeName {
			...
			reserves, err := cfg.Helper.GetReserves(pairAddress.String())
			if err != nil {
				log.Printf("Error getting reserves for exchange %s: %v and the pair address %v\n\n", exchange.Name, err, pairAddress)
				return
			}
